...
import com.faisal.pdfengine.util.Constants.PRELOAD_OFFSET
import com.faisal.pdfengine.util.MathUtils
import com.faisal.pdfengine.util.Util
import java.util.LinkedList
import kotlin.math.abs

/** Figures out which page tiles are visible (plus a preload margin) and asks [PDFView] to render them. */
internal class PagesLoader(private val pdfView: PDFView) {
    ...
    private val preloadOffset = Util.getDP(pdfView.context, PRELOAD_OFFSET)

    private class Holder {
        var row = 0
        var col = 0

        override fun toString() = "Holder{row=$row, col=$col}"
    }
    ...
    private class GridSize {
        ...
        override fun toString() = "GridSize{rows=$rows, cols=$cols}"
    }
    ...
    private class RenderRange {
        ...
        override fun toString() = "RenderRange{page=$page, gridSize=$gridSize, leftTop=$leftTop, rightBottom=$rightBottom}"
    }
    ...
    private fun calculatePartSize(grid: GridSize) {
        pageRelativePartWidth = 1f / grid.cols
        pageRelativePartHeight = 1f / grid.rows
    ...
    private fun getRenderRangeList(firstXOffset: Float, firstYOffset: Float, lastXOffset: Float, lastYOffset: Float): List<RenderRange> {
        ...
        val renderRanges = LinkedList<RenderRange>()
        ...
        for (page in firstPage..lastPage) {
            ...
            getPageColsRows(range.gridSize, range.page) // get the page's grid size: rows and cols
            ...
            if (pdfView.isSwipeVertical) {
                range.leftTop.row = MathUtils.floor(abs(pageFirstYOffset - pdfFile.getPageOffset(range.page, pdfView.zoom)) / rowHeight)
                range.leftTop.col = MathUtils.floor(MathUtils.min(pageFirstXOffset - secondaryOffset, 0f) / colWidth)

                range.rightBottom.row = MathUtils.ceil(abs(pageLastYOffset - pdfFile.getPageOffset(range.page, pdfView.zoom)) / rowHeight)
                range.rightBottom.col = MathUtils.floor(MathUtils.min(pageLastXOffset - secondaryOffset, 0f) / colWidth)
            } else {
                range.leftTop.col = MathUtils.floor(abs(pageFirstXOffset - pdfFile.getPageOffset(range.page, pdfView.zoom)) / colWidth)
                range.leftTop.row = MathUtils.floor(MathUtils.min(pageFirstYOffset - secondaryOffset, 0f) / rowHeight)

                range.rightBottom.col = MathUtils.floor(abs(pageLastXOffset - pdfFile.getPageOffset(range.page, pdfView.zoom)) / colWidth)
        ...
        if (renderWidth > 0 && renderHeight > 0) {
            if (!pdfView.cacheManager.upPartIfContained(page, pageRelativeBounds, cacheOrder)) {
                pdfView.renderingHandler?.addRenderingTask(
                    page, renderWidth, renderHeight, pageRelativeBounds, false, cacheOrder,
